...
def parse_name(component):
    return component[ckw_.component_name]
...
def parse_from_taxonomic_component(component, supported_instances):
    #info("--- Parsing taxonomic component reference")
    name_ = component[ckw_.component_name]
    taxonomic_component = component[ckw_.from_taxonomic_component]
    taxo_component_dict = tr_.getComponent(taxonomic_component)
    taxo_category = taxo_component_dict[ckw_.taxo_component_description]
    taxo_category_name = taxo_category.getName()
    taxo_category_attributes = taxo_category.get_attributes()
    instances = taxo_component_dict[ckw_.taxo_component_instances]
    ...
    #warn("--- => Done, parsing taxonomic component reference")
...
def synth_arch_buffer_action_map(model_instance,buffer_upstream_of_port, \
                                 upstream_action,downstream_action):
    info(".arch_buffer_action_map(buffer_upstream_of_port =",buffer_upstream_of_port,", upstream_action =", \
         upstream_action,", downstream_action =",downstream_action,")")
    model_instance.arch_buffer_action_map(buffer_upstream_of_port=buffer_upstream_of_port, \
                                            upstream_action=upstream_action, \
                                            downstream_action=downstream_action)

def parse_arch_buffer_action_maps(component, model_instance):
    if 'arch_buffer_action_map' in component:
        arch_buffer_action_map_list = component['arch_buffer_action_map']
        for arch_buffer_action_map in arch_buffer_action_map_list:
            buffer_upstream_of_port=arch_buffer_action_map['buffer_upstream_of_port']
            upstream_action=arch_buffer_action_map['upstream_action']
            downstream_action=arch_buffer_action_map['downstream_action']
            synth_arch_buffer_action_map(model_instance,buffer_upstream_of_port, \
                                            upstream_action,downstream_action)
    return model_instance

def parse_actions(component, model_instance):
    for action in component.get('actions', []):
        info(".action(",action['name'],")")
        model_instance.action(action['name'])
    return model_instance

def synth_subaction_call(model_instance,action_id,impl_alias, \
                         subcomp_id,subaction_id):

    info(".subaction(impl_ =",impl_alias,", action_name_ =",action_id, \
         ", sub_component =",subcomp_id,", sub_action =",subaction_id,")")
...
def parse_subactions(component, model_instance):
    for action_struct in component.get('actions', []):
        #info(".action(",action['name'],")")
        #model_instance.action(action['name'])
        if 'implementations' in action_struct:
            action_id=action_struct['name']
            for impl_struct in action_struct['implementations']:
                if 'subcomponents' in impl_struct:
                    impl_alias=impl_struct['alias']
                    for subcomponent in impl_struct['subcomponents']:
...
def parse_characterization_metric_models(component):
    '''Parse characterization metric models'''
    ctbl_dict={}
    chmm_dict={}
    for chmm_spec in component.get('characterization_metric_models', []):
        # Parse
...
def parse_require_port_throughput_attributes(component, model_instance):
    for load_symbol in component.get('load_symbols', []):
...
def parse_export_attributes_to_model(component, model_instance):
    '''
    for load_symbol in component.get('load_symbols', []):
        ...
        for sym_flav_dict in symbol_flavors:
            ...
            if sym_flav=='level':
    '''
    if component.get('export_attributes_to_model', False):
...
def parse_yield_port_throughput_thresholds(component, model_instance):
    port_attr_dict={}
    for load_symbol in component.get('load_symbols', []):
        port,symbol_flavors=parse_load_symbol_port_flavors(load_symbol)
        for sym_flav_dict in symbol_flavors:
...
def parse_instance_aliases(component, model_instance):
    for instance_alias in component.get('instance_aliases', []):
...
def parse_register_supported_instances(component, model_instance, supported_instances):
...
def parse_implementations(component, model_instance):
    
    # Extract the implementations list from the component dictionary.
