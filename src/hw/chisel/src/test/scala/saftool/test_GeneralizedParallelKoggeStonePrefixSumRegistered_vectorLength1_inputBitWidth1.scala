...
class Test_Sim_GeneralizedParallelKoggeStonePrefixSumRegistered extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "GeneralizedParallelKoggeStonePrefixSumRegistered"
  ...
  // Phase 2: Cartesian product of inputBitWidth [2,4,8,16] and vectorLength [1,2,4,8,16]
  ...
  def createTest(vectorLength: Int, inputBitWidth: Int): Unit = {
    val testName = s"test_GeneralizedParallelKoggeStonePrefixSumRegistered_vectorLength${vectorLength}_inputBitWidth${inputBitWidth}.scala"
    ...
    it should s"vectorLength${vectorLength}_inputBitWidth${inputBitWidth}" in {
      test(new GeneralizedParallelKoggeStonePrefixSumRegistered(vectorLength, inputBitWidth))
        .withAnnotations(Seq(WriteVcdAnnotation))
        .runPeekPoke(new Workload_GeneralizedParallelKoggeStonePrefixSumRegistered_Random(_))
    }
    emitVerilog(vectorLength, inputBitWidth)
  }

  def emitVerilog(vectorLength: Int, inputBitWidth: Int): Unit = {
    val filename = s"GeneralizedParallelKoggeStonePrefixSumRegistered_vectorLength${vectorLength}_inputBitWidth${inputBitWidth}.v"
    val verilogString = chisel3.getVerilogString(new GeneralizedParallelKoggeStonePrefixSumRegistered(vectorLength, inputBitWidth))
